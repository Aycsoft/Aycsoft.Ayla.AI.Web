/** 用量协议回归：核对鉴权路径、日期参数及服务商实测行聚合，不构造估算用量。 */
...
describe('usage summary contract', () => {
  it('uses the authenticated workspace summary route and ISO date query', () => {
    ...
    expect(usageSummaryPath({ from: '2026-08-01', to: '2026-08-25' })).toBe(
      '/workspace/usage/summary?from=2026-08-01&to=2026-08-25',
    )
  })
  it('aggregates only provider-reported row values by model', () => {
    ...
    ).toMatchObject({ TotalTokens: 22, MeasuredInvocations: 2 })
