...
/**
 * 工作区导航与会话历史入口，协调分页、路由、账号弹窗和会话操作。
 * open 控制移动端显隐，close 请求收起；数据由会话和认证 Store 管理。
 */
...
const filtered = computed(() =>
  chat.conversations.filter((x) =>
    (x.Title || '').toLowerCase().includes(keyword.value.trim().toLowerCase()),
  ),
)
...
const menuConversation = computed(() =>
  chat.conversations.find((item) => item.Id === conversationMenu.value?.id),
)
const toggleCollapsed = () => {
  collapsed.value = !collapsed.value
  localStorage.setItem('crosscart-ai-sidebar-collapsed', collapsed.value ? '1' : '0')
}
...
const newChat = async () => {
  ...
  await router.replace({
    name: 'chat',
    query: chat.currentId ? { conversation: chat.currentId } : {},
  })
  emit('close')
}
...
const openConversationMenuKeyboard = (
  event: KeyboardEvent,
  ...
  conversationMenu.value = {
    ...
    y: Math.min(rect.bottom, window.innerHeight - 100),
  }
}
const beginAction = (mode: 'rename' | 'delete', item: { Id: string; Title: string }) => {
  conversationMenu.value = undefined
  actionError.value = ''
  actionDialog.value = { mode, id: item.Id, title: item.Title || '新会话' }
}
const confirmAction = async (value: string) => {
  ...
  try {
    ...
    actionError.value = (error as Error).message || '会话操作失败，请稍后重试'
  } finally {
    actionBusy.value = false
  }
}
const openPortal = () => {
  const target = new URL(String(import.meta.env.VITE_SSO_LOGIN_URL || '/'), window.location.origin)
  target.hash = '#/'
  window.location.assign(target)
}
const toggleTheme = () => {
  theme.value = nextTheme(theme.value)
  applyWorkspaceTheme(theme.value)
}
</script>

<template>
  <aside
    :class="['minimax-sidebar', { open, collapsed }]"
    :aria-label="`${settings.brand} 工作区导航`"
  >
    <div class="sidebar-brand">
      <button class="brand-home" :aria-label="`${settings.brand} 首页`" @click="newChat">
        <BrandLogo /><strong>{{ settings.brand }}</strong>
      </button>
      <button
      ...
      <button class="sidebar-mobile-close" aria-label="关闭导航" @click="emit('close')">
        <Icon icon="lucide:x" />
      </button>
    </div>

    <nav class="sidebar-primary">
      <button title="新建任务" @click="newChat">
        <Icon icon="lucide:square-pen" /><span>新建任务</span>
      </button>
      <button title="搜索真实会话" @click="searchOpen = !searchOpen">
        <Icon icon="lucide:search" /><span>搜索</span>
      </button>
      <label v-if="searchOpen && !collapsed" class="sidebar-search"
        ><Icon icon="lucide:search" /><input
          ...
          placeholder="搜索最近任务"
      /></label>
      <button v-if="auth.session" title="管理个人与授权知识库" @click="router.push('/knowledge')">
        <Icon icon="lucide:library-big" /><span>知识库</span>
      </button>
      <button v-if="auth.session" title="管理 AI 输出规范" @click="router.push('/skills')">
        <Icon icon="lucide:wand-sparkles" /><span>Skill</span>
      </button>
      <button v-if="auth.session" title="管理 Markdown 笔记" @click="router.push('/notes')">
        <Icon icon="lucide:notebook-pen" /><span>笔记</span>
      </button>
      <button v-if="!auth.session || isInternal" title="打开 CrossCart Portal" @click="openPortal">
        <Icon icon="lucide:external-link" /><span>打开 Portal</span>
      </button>
    </nav>

    <section v-if="auth.session && isInternal && !collapsed" class="sidebar-group">
      <div class="sidebar-group-title">ERP AGENT</div>
      <button disabled title="需要后端返回真实 Agent 清单">
        <Icon icon="lucide:boxes" /><span>业务 Agent 由服务端配置</span>
      </button>
    </section>

    <section
      v-if="!collapsed"
      class="sidebar-group recent-group"
      :aria-busy="chat.conversationsLoadingMore"
      ...
      <template v-else>
        ...
        <template v-for="group in groupedConversations" v-else :key="group.key">
          ...
          <div
            ...
            :key="item.Id"
            :class="['recent-task', { active: item.Id === chat.currentId }]"
            role="button"
            tabindex="0"
            @click="select(item.Id)"
            @keydown.enter="select(item.Id)"
            @dblclick.prevent.stop="beginAction('rename', item)"
            @contextmenu="openConversationMenu($event, item)"
          >
            <span
              v-if="chat.conversationStatuses[item.Id] === 'running'"
              class="conversation-run-status running"
              title="正在生成"
              ><Icon icon="lucide:loader-circle" /></span
            ><Icon v-else icon="lucide:message-square" /><span>{{ item.Title || '新会话' }}</span>
            <button
              class="task-menu-trigger"
              title="会话操作"
              aria-label="会话操作"
              @click.stop="openConversationMenu($event, item)"
              @keydown.enter="openConversationMenuKeyboard($event, item)"
            >
        ...
        <button
          v-if="chat.conversationsHasMore"
          class="conversation-load-more"
          type="button"
          :disabled="chat.conversationsLoadingMore"
          @click="loadMoreConversations"
        >
          <Icon
            :icon="chat.conversationsLoadingMore ? 'lucide:loader-circle' : 'lucide:chevrons-down'"
          />
    ...
    <div class="sidebar-account">
      <button
        :title="auth.session?.UserName || '登录'"
        @click="auth.session ? (accountOpen = !accountOpen) : auth.login()"
      >
        <span class="account-avatar"
          ><Icon :icon="auth.session ? 'lucide:user-round' : 'lucide:log-in'" /><img
            v-if="avatarContentUrl"
            :src="avatarContentUrl"
            alt="个人头像"
            @error="($event.currentTarget as HTMLImageElement).remove()"
        /></span>
        <span class="account-copy"
          ><strong>{{ auth.session?.UserName || '登录' }}</strong
          ><small>{{
            auth.session ? (isInternal ? '企业内部账号' : '外部 AI 账号') : '内部 / 外部用户登录'
          }}</small></span
        >
      ...
      <div v-if="accountOpen && auth.session && !collapsed" class="account-popover">
        ...
        ><small>{{
          isInternal
            ? `${auth.session.Permissions.length} 项企业权限`
            : '会话、文件与媒体能力已开放'
        }}</small>
        <button v-if="isInternal" @click="openPortal">
          <Icon icon="lucide:external-link" />进入 Portal
        </button>
        <button v-if="!isInternal" @click="openProfile">
          <Icon icon="lucide:user-round-cog" />个人中心
        </button>
        <button @click="openUsage"><Icon icon="lucide:gauge" />AI 用量</button>
  ...
  <ExternalPasswordDialog
    v-if="auth.session?.AccountType === 'external'"
    :open="passwordDialogOpen"
    :has-password="auth.session.HasPassword"
    @close="passwordDialogOpen = false"
    @saved="auth.refreshSession()"
  />
  ...
  <Teleport to="body"
    ><div
      ...
      @click="conversationMenu = undefined"
      @contextmenu.prevent="conversationMenu = undefined"
    />
    <div
      v-if="conversationMenu && menuConversation"
      class="conversation-context-menu"
      :style="{ left: `${conversationMenu.x}px`, top: `${conversationMenu.y}px` }"
    >
      <button @click="beginAction('rename', menuConversation)">
        <Icon icon="lucide:pencil-line" />重命名</button
      ><button class="danger" @click="beginAction('delete', menuConversation)">
