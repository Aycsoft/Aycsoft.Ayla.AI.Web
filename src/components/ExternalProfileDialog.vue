...
/**
 * 外部账号资料编辑器，先上传头像取得文件 ID，再提交资料关联。
 * session 提供初始快照；saved 请求刷新，password 切换密码表单，close 请求关闭。
 */
...
async function uploadAvatar(files: FileList | null) {
  ...
  if (
    ...
    error.value = '头像仅支持 PNG、JPG、WebP，大小不超过 5MB'
    return
  }
  ...
  try {
    ...
    error.value = reason instanceof ApiError ? reason.message : '头像上传失败'
  } finally {
...
async function save() {
  ...
  error.value = ''
  try {
    await aiApi.updateExternalProfile({
    ...
    error.value = reason instanceof ApiError ? reason.message : '个人资料保存失败'
  } finally {
    saving.value = false
  }
}
...
<template>
  <Teleport to="body">
    <div v-if="open" class="password-dialog-backdrop" @click.self="emit('close')">
      <section
        class="password-dialog profile-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <span
            ><Icon icon="lucide:user-round-cog" /><strong id="profile-dialog-title"
              >个人中心</strong
            ></span
          ><button aria-label="关闭" @click="emit('close')"><Icon icon="lucide:x" /></button>
        </header>
        <div class="profile-avatar-editor">
          <button :disabled="uploading || saving" @click="picker?.click()">
            <img v-if="avatarPreview" :src="avatarPreview" alt="当前头像" /><span v-else>{{
              (name || 'A').slice(0, 1).toUpperCase()
            }}</span
            ><i><Icon :icon="uploading ? 'lucide:loader-circle' : 'lucide:camera'" /></i>
          </button>
          <div>
            <strong>个人头像</strong><small>PNG、JPG、WebP · 最大 5MB</small
            ><button :disabled="uploading || saving" @click="picker?.click()">
              {{ uploading ? '上传中…' : '更换头像' }}
            </button>
          </div>
          <input
            ...
            @change="uploadAvatar(($event.target as HTMLInputElement).files)"
          />
        </div>
        ...
        <label class="auth-field"
          ...
          <div class="profile-readonly">
            <Icon icon="lucide:mail" /><input :value="session.Email || ''" readonly /></div
        ></label>
        <button class="profile-password-entry" @click="emit('password')">
          <Icon icon="lucide:key-round" /><span
            ><strong>{{ session.HasPassword ? '修改登录密码' : '设置登录密码' }}</strong
            ><small>管理外部账号的密码登录凭证</small></span
        ...
        <footer>
          <button class="password-cancel" :disabled="saving" @click="emit('close')">取消</button
          ><button
            class="auth-primary"
