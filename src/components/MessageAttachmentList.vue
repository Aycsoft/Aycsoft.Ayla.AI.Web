...
const markFailed = (file: Attachment, message = '附件加载失败，请重试') => { failed.value[file.FileId] = message }
const retry = (file: Attachment) => { delete failed.value[file.FileId]; retryKeys.value[file.FileId] = (retryKeys.value[file.FileId] || 0) + 1 }
const openPreview = (file: Attachment) => { if (!contentUrl(file)) markFailed(file, '文件服务未返回可预览地址'); else preview.value = file }
const download = async (file: Attachment) => { try { const source = downloadUrl(file); if (!source) throw new Error('missing download url'); await downloadBackendResource(source, file.FileName); retry(file) } catch { markFailed(file, '附件下载失败，请重试') } }
</script>
<template>
  ...
  <ResourcePreviewDialog v-if="preview" :open="true" :file-name="preview.FileName" :content-type="preview.ContentType" :content-url="contentUrl(preview) || undefined" :download-url="downloadUrl(preview) || undefined" @close="preview = undefined" />
</template>
