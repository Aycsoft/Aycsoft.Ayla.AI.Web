/** 执行时间线回归：核对工具分类、公开摘要脱敏、真实计数、终态计时和历史映射。 */
...
describe('execution trace presentation', () => {
  it('classifies only real tool codes and never exposes private reasoning detail', () => {
    expect(
      executionStepKind({ StepId: '1', ToolCode: 'web_search', Title: '检索', Status: 'running' }),
    ).toBe('search')
    expect(
      executionStepKind({
        StepId: '2',
        ToolCode: 'attachment-read',
        Title: '读取',
        Status: 'completed',
      }),
    ).toBe('read')
    ...
    expect(
      executionStepSummary({
        StepId: '3',
        ToolCode: 'deep_reasoning',
        Title: '系统提示词: secret',
        Detail: 'private chain',
        Status: 'completed',
      }),
    ).toBe('已完成问题分析与回答组织')
    expect(
      executionStepSummary({
        StepId: '4',
        ToolCode: 'private_tool_code',
        Title: 'private_tool_code',
        Status: 'running',
      }),
    ).toBe('正在执行授权操作')
    expect(
    ...
    expect(
      executionStepLabel({
        StepId: '6',
        ToolCode: 'evidence_verification',
        Title: '已核对官方文档',
        Status: 'completed',
      }),
    ).toBe('Verify')
  ...
  it('keeps elapsed time moving only for a running event', () => {
    expect(
      executionElapsedMs(
        { StepId: '1', Title: 'run', Status: 'running', StartedAtMs: 1_000 },
        3_500,
      ),
    ).toBe(2_500)
    expect(
      executionElapsedMs(
        { StepId: '2', Title: 'done', Status: 'completed', StartedAtMs: 1_000, ElapsedMs: 800 },
        3_500,
      ),
    ).toBe(800)
  })
  ...
  it('normalizes persisted ToolSteps without inventing steps', () => {
    const result = normalizePersistedTraceSteps([
      {
        InvocationId: 'tool-1',
        ToolCode: 'web_search',
        Stage: 'completed',
        Message: '已获得 3 条互联网证据',
        ElapsedMs: 1200,
      },
    ])
