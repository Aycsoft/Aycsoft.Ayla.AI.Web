...
describe('media inspiration empty-state contract', () => {
  ...
  it('covers reference-image creation modes explicitly', () => {
    expect(mediaInspirationFor('image').some(item => item.generationMode === 'image-to-image')).toBe(true)
    expect(mediaInspirationFor('video').some(item => item.generationMode === 'image-to-video')).toBe(true)
    expect(mediaInspirationFor('video').some(item => item.generationMode === 'keyframe-video')).toBe(true)
  })
})
