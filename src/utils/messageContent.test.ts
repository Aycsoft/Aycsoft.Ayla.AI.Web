/** 消息输出回归：验证围栏分段、流式未闭合代码、产物分类及 JSON/预览限制；不证明浏览器隔离。 */
...
describe('message output contracts', () => {
  ...
  it('runs JSON validation without executing arbitrary code', async () => {
    ...
    await expect(runCodeSafely('json', '{"ok":true}')).resolves.toEqual({
      ok: true,
      output: '{\n  "ok": true\n}',
    })
    const preview = buildSandboxPreview(
      'html',
      '<h1>safe</h1><script>document.body.dataset.ok="1"</script>',
    )!
