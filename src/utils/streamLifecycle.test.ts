/** 流生命周期回归：验证稳定占位、真实步骤替换和终态收口，不覆盖既有完成结果。 */
import { describe, expect, it } from 'vitest'
import {
  createGenerationPlaceholder,
  generationPlaceholderId,
...
describe('stream lifecycle', () => {
  ...
  it('replaces the placeholder when real agent steps arrive', () => {
    const traces = [
      createGenerationPlaceholder(),
      { StepId: 'plan', Title: '计划', Status: 'completed' as const },
    ]
  ...
  it('settles every pending spinner at terminal state', () => {
    const traces = [
      createGenerationPlaceholder(),
      { StepId: 'tool', Title: '工具', Status: 'running' as const },
      { StepId: 'done', Title: '完成', Status: 'completed' as const },
    ]
