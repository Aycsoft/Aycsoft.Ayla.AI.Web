...
/** 企业 SSO 与外部账号登录入口，统一组织验证码、注册和密码重置交互。 */
...
import {
  ...
  externalUserPurposes,
  isValidEmail,
  normalizeEmail,
} from '@/utils/externalAuth'
...
const loginUrl = String(
  import.meta.env.VITE_SSO_LOGIN_URL || `${portalOrigin}/#/ai/sso/authorize`,
).trim()
const internalConfigured = computed(() => Boolean(loginUrl))
const codeLabel = computed(() =>
  countdown.value > 0 ? `${countdown.value} 秒后重发` : '获取验证码',
)
const codePurpose = computed<ExternalAuthScene>(() =>
  resettingPassword.value ? 'reset-password' : scene.value,
)
...
// 根据当前场景集中定义提交条件，避免按钮提示与实际提交校验分叉。
...
// 身份通道/登录方式切换时清空敏感字段；卸载时释放验证码倒计时。
watch([channel, scene, loginMethod], () => {
  error.value = ''
  code.value = ''
  password.value = ''
...
/** 注册使用目的支持多选，其他用途另行填写说明。 */
function togglePurpose(value: string) {
  purposes.value = purposes.value.includes(value)
    ? purposes.value.filter((item) => item !== value)
    : [...purposes.value, value]
}

/** 在登录和注册之间切换时退出密码重置；其他敏感字段由场景 watch 统一清空。 */
function selectAuthScene(value: 'login' | 'register') {
  scene.value = value
  resettingPassword.value = false
}

/** 进入密码重置前清除之前的凭证，保留用户已填写的邮箱。 */
...
/** 发送当前业务场景验证码，防止连续点击绕过请求中状态。 */
async function sendCode() {
  if (sendingCode.value || countdown.value > 0) return
  error.value = ''
  if (!isValidEmail(email.value)) {
    error.value = '请输入可接收验证码的有效邮箱地址'
    return
  }
  sendingCode.value = true
  try {
    const response = await aiApi.sendExternalAuthCode({
...
/** 校验 → 按场景认证/重置 → 回到原会话；任何失败都保留表单便于修正。 */
async function submitExternal() {
  if (submitting.value) return
  error.value = ''
  if (submitIssue.value) {
    error.value = submitIssue.value
    return
  }
  submitting.value = true
  try {
    if (resettingPassword.value) {
      await aiApi.resetExternalPassword({
        Email: normalizeEmail(email.value),
    ...
    if (scene.value === 'register')
      ...
      await auth.loginExternalPassword({
        Email: normalizeEmail(email.value),
        Password: password.value,
      })
    ...
    await router.replace({
      name: 'chat',
      query: route.query.conversationId ? { conversation: String(route.query.conversationId) } : {},
    })
  } catch (reason) {
  ...
  <main class="auth-page">
    <section class="auth-visual" aria-label="AI 工作台能力说明">
      <div class="auth-brand">
        <BrandLogo :size="34" /><strong>{{ settings.brand }}</strong>
      </div>
      <div class="auth-visual-copy">
        ...
        <div class="auth-boundary">
          <Icon icon="lucide:shield-check" /><span
            ><strong>数据边界清晰</strong
            ><small>外部账号无法访问 CrossCart ERP 业务数据，其他 AI 能力完整开放。</small></span
          >
        </div>
      </div>
    ...
    <section class="auth-panel">
      <div class="auth-card">
        ...
        <div class="auth-channel-tabs" role="tablist" aria-label="账号类型">
          <button
            :class="{ active: channel === 'internal' }"
            role="tab"
            :aria-selected="channel === 'internal'"
            @click="channel = 'internal'"
          >
            <Icon icon="lucide:building-2" /><span
              ><strong>企业内部用户</strong><small>使用 CrossCart ERP 账号</small></span
            >
          </button>
          <button
            :class="{ active: channel === 'external' }"
            role="tab"
            :aria-selected="channel === 'external'"
            @click="channel = 'external'"
          >
            <Icon icon="lucide:users-round" /><span
              ><strong>外部用户</strong><small>使用邮箱或密码</small></span
            >
          </button>
        </div>
        ...
        <div v-if="channel === 'internal'" class="internal-auth">
          <div class="internal-provider">
            <div class="internal-provider-brand">
              <span><Icon icon="lucide:building-2" /></span>
              <div>
                <strong>CrossCart 企业账号</strong><small>统一身份认证 · 组织权限自动同步</small>
              </div>
              <Icon icon="lucide:badge-check" />
            </div>
            <ol class="internal-auth-flow">
              <li><span>1</span>前往 ERP 登录</li>
              <li><span>2</span>确认企业身份</li>
              <li><span>3</span>返回当前工作台</li>
            </ol>
          ...
          <button
            class="auth-primary"
            type="button"
            :disabled="!internalConfigured"
            @click="enterPortal"
          >
            <span>使用企业账号继续</span><Icon icon="lucide:arrow-right" />
          </button>
          <p v-if="!internalConfigured" class="auth-error">
            <Icon icon="lucide:triangle-alert" /> 当前环境尚未配置 ERP 统一登录地址。
          </p>
        </div>
        ...
        <form v-else class="external-auth" @submit.prevent="submitExternal">
          <div class="auth-mode-tabs" role="tablist" aria-label="外部账号操作">
            <button
              type="button"
              :class="{ active: scene === 'login' }"
              @click="selectAuthScene('login')"
            >
              账号登录
            </button>
            <button
              type="button"
              :class="{ active: scene === 'register' }"
              @click="selectAuthScene('register')"
            >
              创建账号
            </button>
          </div>
          <div
            v-if="scene === 'login' && !resettingPassword"
            class="auth-login-methods"
            role="tablist"
            aria-label="登录凭证"
          >
            <button
            ...
            >
              <Icon icon="lucide:key-round" />密码登录
            </button>
            <button
              type="button"
            ...
            >
              <Icon icon="lucide:mail-check" />验证码登录
            </button>
          </div>
          <div v-if="resettingPassword" class="auth-reset-heading">
            <button type="button" aria-label="返回登录" @click="resettingPassword = false">
              <Icon icon="lucide:arrow-left" /></button
            ><span><strong>重置登录密码</strong><small>验证码将发送至您的注册邮箱</small></span>
          </div>
          <label v-if="scene === 'register'" class="auth-field"
            ><span>您的名称</span>
            <div>
              <Icon icon="lucide:user-round" /><input
                v-model="name"
                maxlength="50"
                autocomplete="name"
                placeholder="请输入您的名称"
          ...
          ></label>
          <label
            v-if="scene === 'register' || resettingPassword || loginMethod === 'code'"
            class="auth-field"
            ><span>邮箱验证码</span>
            <div class="auth-code-field">
              <Icon icon="lucide:badge-check" /><input
                v-model="code"
                inputmode="numeric"
                maxlength="8"
              ...
              /><button type="button" :disabled="sendingCode || countdown > 0" @click="sendCode">
                {{ sendingCode ? '发送中…' : codeLabel }}
              </button>
            </div></label
          >
          <label
            v-if="scene === 'register' || resettingPassword || loginMethod === 'password'"
            class="auth-field"
            ><span>{{
              scene === 'register' ? '设置登录密码' : resettingPassword ? '新密码' : '登录密码'
            }}</span>
            <div class="auth-password-field">
              <Icon icon="lucide:key-round" /><input
                v-model="password"
                :type="showPassword ? 'text' : 'password'"
                maxlength="128"
                :autocomplete="
                  scene === 'login' && !resettingPassword ? 'current-password' : 'new-password'
                "
                :placeholder="
                  scene === 'login' && !resettingPassword
                    ? '请输入登录密码'
                    : '至少 10 位，包含三类字符'
                "
              /><button
          ...
          ></label>
          <label v-if="scene === 'register' || resettingPassword" class="auth-field"
            ><span>确认密码</span>
            <div>
          ...
          ></label>
          <div
            v-if="scene === 'register' || resettingPassword"
            class="auth-password-status"
            aria-live="polite"
          >
            <span
              :class="{ valid: passwordStrong, invalid: password.length > 0 && !passwordStrong }"
              ><Icon :icon="passwordStrong ? 'lucide:circle-check' : 'lucide:circle'" />至少 10
              位，包含四类字符中的三类</span
            >
            <span
              :class="{
              ...
              ><Icon
                :icon="passwordsMatch ? 'lucide:circle-check' : 'lucide:circle'"
              />两次密码一致</span
            >
          </div>
          <button
            v-if="scene === 'login' && loginMethod === 'password' && !resettingPassword"
            class="auth-forgot"
            type="button"
          ...
          <fieldset v-if="scene === 'register'" class="purpose-fieldset">
            ...
            <div class="purpose-grid">
              <button
                v-for="item in externalUserPurposes"
                :key="item.value"
                type="button"
                :class="{ selected: purposes.includes(item.value) }"
                :aria-pressed="purposes.includes(item.value)"
                @click="togglePurpose(item.value)"
              >
                <Icon :icon="item.icon" /><span>{{ item.label }}</span
                ><Icon
                  :icon="
                    purposes.includes(item.value) ? 'lucide:circle-check-big' : 'lucide:circle'
                  "
                />
              </button>
            </div>
            <label v-if="purposes.includes('other')" class="auth-field other-purpose"
              ><span>其他使用目的</span>
              <div>
                <Icon icon="lucide:message-square-text" /><input
          ...
          <button class="auth-primary" type="submit" :disabled="submitting || !canSubmit">
            <span>{{
              submitting
                ? '正在验证…'
                : resettingPassword
            ...
            ><Icon :icon="submitting ? 'lucide:loader-circle' : 'lucide:arrow-right'" />
          </button>
        </form>
      </div>
    </section>
